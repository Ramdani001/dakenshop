...
const ProductsPage = () => {
  ...
  const [buyShippingDuration, setBuyShippingDuration] =
    useState("Reguler (2-3 Hari)");
  ...
  const fetchData = async () => {
    try {
      ...
      const [resProducts, resCategories] = await Promise.all([
        fetch(`${CONFIG.BASE_URL}/api/products?page=1&limit=100`),
        fetch(`${CONFIG.BASE_URL}/api/categories?page=1&limit=100`),
      ]);
  ...
  const handleAddToCart = async () => {
    ...
    if (!token) {
      alert(
        "Silakan login akun terlebih dahulu untuk menggunakan fitur keranjang!",
      );
      return;
    }
  ...
  const handleCheckoutAndWhatsApp = async (e) => {
    ...
    if (!token) {
      alert(
        "Silakan login akun terlebih dahulu untuk melakukan transaksi pembelian!",
      );
      return;
    }
...
_Sistem otomatis: Data pesanan ini telah disinkronkan ke database transaksi DakenShop._`;
  ...
  const FilterContent = () => (
    <div className="filter-sidebar-content text-start">
      <div className="mb-4">
        <span
          className="filter-label fw-bold d-block mb-3 border-bottom pb-2 text-uppercase"
  ...
  return (
    <div
      className="products-page min-vh-100 d-flex flex-column"
      style={{ paddingTop: "100px", backgroundColor: "#f8f9fa" }}
    >
              ...
              <>
                ...
                {filteredProducts.length > 0 ? (
                    ...
                    {filteredProducts.map((product) => {
                      const basePrice =
                        product.types && product.types.length > 0
                          ? Number(product.types[0].price)
                          : 0;
                      const discountPercent =
                        Number(product.discountPercentage) || 0;
                      const finalCatalogPrice =
                      ...
                      return (
                        ...
                        >
                          <Card
                            className="product-card shadow-sm h-100 border-0 overflow-hidden"
                          ...
                          >
                            <div className="product-img-container position-relative">
                              ...
                              {displayThumbnail ? (
                                <img
                                  src={`${CONFIG.BASE_URL}${displayThumbnail}`}
                            ...
                            <Card.Body className="p-3 text-center d-flex flex-column">
                              ...
                              <Button
                                variant="dark"
                                className="mt-auto btn-sm fw-bold rounded-pill"
                              >
                                Detail Produk
                              </Button>
                            </Card.Body>
                  ...
                  <div className="text-center py-5 bg-white rounded-3 shadow-sm mt-2">
                    ...
                    <p className="text-muted">
                      Maaf, saat ini belum ada produk untuk kategori{" "}
      ...
      <Modal
        show={showDetail}
        onHide={() => !isAddingToCart && setShowDetail(false)}
        size="lg"
        centered
      >
        <Modal.Body className="p-0 position-relative">
          <Button
            ...
            className="position-absolute end-0 top-0 m-3 z-3 rounded-circle"
          ...
          {selectedProduct && (
            <Row className="g-0">
              ...
              >
                {(() => {
                  const modalImages = parseProductImages(
                    selectedProduct.imgUrl,
                  );
                  return modalImages[0] ? (
                    <img
                      src={`${CONFIG.BASE_URL}${modalImages[0]}`}
              ...
              >
                <div>
                  ...
                  <div className="bg-light p-3 rounded-3 mb-4 border">
                    {selectedVariant ? (
                      <>
                        ...
                        <h2
                          className="text-danger fw-extrabold m-0"
                        ...
                        >
                          ...
                          {(
                            ...
                            (1 -
                              Number(selectedProduct.discountPercentage) / 100)
                  ...
                  <div className="mb-4">
                    <span
                      className="small d-block fw-bold text-secondary text-uppercase mb-2"
                    ...
                    <div className="d-flex flex-wrap gap-2">
                      {selectedProduct.types?.map((t, idx) => {
                        const isSelected =
                          selectedVariant && selectedVariant.id === t.id;
                        return (
                          <button
                            key={idx}
                            type="button"
                          ...
                          >
                            <div
                              className={`fw-semibold ${isSelected ? "text-primary" : "text-secondary"}`}
                              style={{ fontSize: "11px" }}
                            >
                              {t.type}
                            </div>
                ...
                <div className="d-grid gap-2 mt-4">
                  <Button
                    variant="outline-dark"
                    size="lg"
                    className="fw-bold py-2.5 d-flex align-items-center justify-content-center gap-2"
                  ...
                  >
                    {isAddingToCart ? (
                      ...
                      <>
                        <CartPlusFill size={18} /> ADD TO CART
                      </>
                    )}
                  </Button>
                  <Button
      ...
      <Modal
        ...
        onHide={() => !isSubmittingOrder && setShowBuyNowModal(false)}
        size="md"
        centered
        backdrop="static"
      >
            ...
            {selectedProduct && selectedVariant && (
              <>
                <div className="bg-light p-3 rounded-3 border mb-3 small shadow-sm">
                  ...
                  <div
                    className="fw-bold text-dark text-uppercase mt-1"
                    style={{ fontSize: "14px" }}
                  >
                    {selectedProduct.name} ({selectedVariant.type})
                  </div>
                  <div className="fw-bold text-primary h6 m-0 mt-1">
                    Harga Satuan: Rp{" "}
                    {(
                      Number(selectedVariant.price) *
                      (1 - Number(selectedProduct.discountPercentage) / 100)
                    ).toLocaleString("id-ID")}
                  </div>
                </div>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="email"
                    required
                    value={buyCustomerEmail}
                    onChange={(e) => setBuyCustomerEmail(e.target.value)}
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    type="tel"
                    required
                    value={buyCustomerPhone}
                    onChange={(e) => setBuyCustomerPhone(e.target.value)}
                ...
                <Row>
                  ...
                  <Col md={6} xs={12}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Select
                        value={buyShippingDuration}
                        onChange={(e) => setBuyShippingDuration(e.target.value)}
                      >
                        <option value="Reguler (2-3 Hari)">
                          Reguler (2-3 Hari Kerja)
                        </option>
                        <option value="Ekspres (1 Hari)">
                          Ekspres / Same-Day (1 Hari)
                        </option>
                        <option value="Kargo (5-7 Hari)">
                          Kargo / Ekonomi (5-7 Hari)
                        </option>
                      </Form.Select>
                    </Form.Group>
                  </Col>
                ...
                <Form.Group className="mb-3">
                  <Form.Label className="small fw-bold text-secondary">
                    Alamat Tujuan Pengiriman Lengkap
                  </Form.Label>
                  <Form.Control
                    as="textarea"
                    rows={3}
