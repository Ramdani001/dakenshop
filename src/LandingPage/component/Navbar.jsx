...
import { Navbar, Nav, Container, Image, Offcanvas, Button, Spinner } from 'react-bootstrap';
...
const CustomNavbar = () => {
    ...
    try {
      ...
      if (existingCart) {
        ...
        const validCart = parsedCart.filter(item => {
          const targetPrice = item.priceAfterDiscount || item.price;
          return targetPrice !== undefined && !isNaN(Number(targetPrice));
        });
  ...
  const handlePaymentCheckout = async () => {
    if (!isLoggedIn) {
      alert("Silakan login terlebih dahulu untuk melakukan transaksi pembayaran.");
      return;
    }
    ...
    try {
      ...
      if (!snapToken) {
        throw new Error("Gagal memperoleh payment gateway token dari Midtrans.");
      }
  ...
  const totalEstimasiHarga = cartItems.reduce(
    (acc, item) => acc + (Number(item.priceAfterDiscount || item.price) * item.quantity), 
    0
  );

  return (
    <Navbar expanded={expanded} onToggle={setExpanded} collapseOnSelect bg="white" expand="lg" className="border-bottom shadow-sm" fixed="top">
      <Container>
        <Navbar.Brand as={Link} to="/" className="d-flex align-items-center">
          <Image src="/images/daken-shop-logo.png" alt="Logo DakenShop" style={{ width: '45px', height: 'auto' }} className="me-2" />
        ...
        <Navbar.Collapse id="basic-navbar-nav">
          <Nav className="mx-auto fw-bold text-center py-3 py-lg-0">
            <Nav.Link as={Link} to="/" onClick={() => setExpanded(false)} className={currentPath === "/" ? "text-primary" : "text-black"}>BERANDA</Nav.Link>
            <Nav.Link as={Link} to="/produk" onClick={() => setExpanded(false)} className={currentPath === "/produk" ? "text-primary" : "text-black"}>PRODUK</Nav.Link>
            <Nav.Link as={Link} to="/#tentang" onClick={() => setExpanded(false)} className={currentPath === "/tentang" ? "text-primary" : "text-black"}>TENTANG KAMI</Nav.Link>
            <Nav.Link as={Link} to="#faq" onClick={() => setExpanded(false)} className={currentPath === "/faq" ? "text-primary" : "text-black"}>FAQ</Nav.Link>
          </Nav>
        </Navbar.Collapse>
      </Container>
              ...
              cartItems.map((item, idx) => (
                <div key={idx} className="d-flex align-items-start bg-white p-3 border rounded-3 mb-2 shadow-sm">
                  <div style={{ width: "65px", height: "65px" }} className="bg-light rounded overflow-hidden flex-shrink-0 border">
                    {item.imgUrl ? (
                      <img src={item.imgUrl.startsWith("http") ? item.imgUrl : `${BASE_URL}${item.imgUrl}`} alt="item" className="w-100 h-100" style={{ objectFit: "cover" }} />
                    ) : (
                  ...
                  <div className="ms-3 flex-grow-1 text-start">
                    ...
                    <div className="text-muted font-monospace" style={{ fontSize: "11px" }}>Varian: {item.variantType || item.type}</div>
                    <div className="mt-1">
                      <div className="fw-bold text-primary" style={{ fontSize: "14px" }}>
                        Rp {Number(item.priceAfterDiscount || item.price).toLocaleString("id-ID")}
                      </div>
                    </div>

                    <div className="d-flex align-items-center bg-light border rounded p-0.5 mt-2" style={{ width: 'fit-content' }}>
                      <Button variant="white" className="border-0 bg-transparent p-1 px-2 text-secondary d-flex align-items-center" onClick={() => handleUpdateQuantity(item.productId, item.variantId, -1)}>
                        <DashLg size={12} />
                      </Button>
                      <span className="fw-bold px-2 text-dark font-monospace" style={{ fontSize: '13px', minWidth: '25px', textAlign: 'center' }}>{item.quantity}</span>
                      <Button variant="white" className="border-0 bg-transparent p-1 px-2 text-secondary d-flex align-items-center" onClick={() => handleUpdateQuantity(item.productId, item.variantId, 1)}>
                        <PlusLg size={12} />
                      </Button>
                    </div>
                  ...
                  <Button variant="link" className="text-danger p-1 ms-2" onClick={() => handleRemoveItem(item.productId, item.variantId)}>
                    <Trash3 size={15} />
                  </Button>
                </div>
              ))
